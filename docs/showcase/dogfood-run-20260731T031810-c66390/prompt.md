...
M DEV.md
 ...
 M tests/test_fixtures_and_offline.py
```

### recent log
```
7efbf1c R6: claim-index anti-restate (sig tokens, whole-file filter, scrub)
d398c8a Dogfood R5: doctor live proof, Opus showcase, R1–R5 milestone
2526196 R5: devmemory doctor for hermes, key, sessions, model
57a6994 Dogfood R4: live Opus showcase with preview.diff, eval, knowledge
b9c4276 R4: unified knowledge git-style preview diff before apply
```
...
@@ -31,7 +29,6 @@ Install once per project (or use `--user` for all repos):
...
@@ -44,4 +41,3 @@ Install once per project (or use `--user` for all repos):
...
diff --git a/tests/test_fixtures_and_offline.py b/tests/test_fixtures_and_offline.py
index d914d12..224bc0c 100644
--- a/tests/test_fixtures_and_offline.py
+++ b/tests/test_fixtures_and_offline.py
@@ -49,3 +49,74 @@ def test_offline_extract_apply(tmp_path: Path):
...
- [DEV.md#Pitfalls] avoid context issues## knowledge large localiz maintainability monolithic
- [DEV.md#Pitfalls] docseval knowledge
...
- [src/auth/DEV.md#Design decisions] currently hs256 later## migrate pattern rs256 token
- [src/auth/DEV.md#Design decisions] decorator protect requireauth
- [src/auth/DEV.md#Design decisions] authoriz [REDACTED] header obtain pitfall token##
- [src/auth/DEV.md#Design decisions] authoriz avoid contain header secret
...
- [src/devmemory/DEV.md#Architecture] assembleextractnormalizeapplytotal decision design extractoutcome## extractsession per-stage record surfac
- [src/devmemory/DEV.md#Architecture] apply assemble extract extraction normalize pipeline
- [src/devmemory/DEV.md#Architecture] dependency includ source vendor
- [src/devmemory/DEV.md#Architecture] extraction openrouter provider
- [src/devmemory/DEV.md#Architecture] develop entirely extraction knowledge locally machine never operat
- [src/devmemory/DEV.md#Architecture] accessible colocat context-specific devmd document easily knowledge maintain
- [src/devmemory/DEV.md#Architecture] artifac
...
- `_norm_bullet` strips quotes, underscores and punctuation (`_'".,;:()[]{}`) in addition to backticks/asterisks so restated bullets normalize to the same string.
- `_token_set` applies a longest-suffix-first `_stem` (ations/ation/tions/ing/ers/ies/ed/es/s) and drops tokens ≤2 chars; the stem guard requires `len(t) > len(suf) + 3` to avoid over-stemming words like "existing".
- `devmemory list-sessions` lists Claude rows before fixture rows, adds a `turns` column from `meta`, and prints a `claude=<n> unprocessed_claude=<n> shown=<n>` summary line so it is obvious
...
- Hook silent? Check `.devmemory/hooks.log`; ensure `devmemory` is on PATH or `.venv/bin/devmemory` exists; set `DEVMEMORY_HOOK_VERBOSE=1` for stderr.
- Hook skipped on Stop: expected unless `DEVMEMORY_
… [truncated; do not restate] …
